...
class DRRGCN_Message_Passing(nn.Module):
    def __init__(self, args, k, n_ntype, n_etype, input_size, hidden_size, output_size,
        ...
        self.k = k
        self.gnn_layers = nn.ModuleList([DRRGCN(hidden_size, n_ntype, n_etype, self.edge_encoder) for _ in range(k)])
    ...
    ### chen start
    def mp_helper(self, _X, edge_index, edge_type, _node_type, _node_feature_extra, node_score):
        for _ in range(self.k):
            _X = self.gnn_layers[_](_X, edge_index, edge_type, _node_type, _node_feature_extra, node_score)
            _X = self.activation(_X)
            _X = F.dropout(_X, self.dropout_rate, training = self.training)
        return _X
    ### chen end
    ...
    def forward(self, H, A, node_type, node_score, cache_output=False):
        ...
        ## chen start
        _X = self.mp_helper(_X, edge_index, edge_type, _node_type, _node_feature_extra, node_score)
        ## chen end
...
class QAGNN(nn.Module):
    def __init__(self, args, k, n_ntype, n_etype, sent_dim,
        ...
        self.gnn = DRRGCN_Message_Passing(args, k=k, n_ntype=n_ntype, n_etype=n_etype,
...
class DRRGCN(MessagePassing):
    ...
    def __init__(self, emb_dim, n_ntype, n_etype, edge_encoder, head_count=4, aggr="add"):
        super(DRRGCN, self).__init__(aggr=aggr)
        assert emb_dim % 2 == 0
    ...
    def forward(self, x, edge_index, edge_type, node_type, node_feature_extra, node_score, return_attention_weights=False):
        # x: [N, emb_dim]
        # edge_index: [2, E]
        ...
        x = torch.cat([x, node_feature_extra], dim=1) ### torch.Size([1000, 400])
        # x = (x, x)
        ...
        relate_cross = relate_cross.view(x.size(0), x.size(0))
        x = torch.matmul(relate_cross, x) # torch.Size([1000, 400])
        x = (x, x)
        aggr_out = self.propagate(edge_index, x=x, edge_attr=edge_embeddings) #[N, emb_dim]
        ...
        ### chen start
        ### self.propagate comes from torch_geometric.nn.MessagePassing
        ### because the DRRGCN class inherit from MessagePassing class
        # print(self.propagate)
        # <bound method MessagePassing.propagate of DRRGCN(
        ...
        # )
        # )>
        # print(node_score.size(), x.size())  ## torch.Size([5, 200, 1]) torch.Size([1000, 400])

        # x = node_score.view(-1,1) * x ### scala and tensor broadcasting
        # # print(x.size()) ## torch.Size([1000, 400])
        # x = (x, x)
        # aggr_out = self.propagate(edge_index, x=x, edge_attr=edge_embeddings) #[N, emb_dim]
        ### chen end
